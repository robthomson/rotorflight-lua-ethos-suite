    ...
    for _, s in pairs(sidList) do
        ...
        if sid then
            ...
            if type(sid) == "table" then
                for i = 1, #sid do elrs.RFSensors[sid[i]] = {name = s.name, unit = s.unit, prec = s.prec, min = s.min, max = s.max, dec = decFn} end
            else
                elrs.RFSensors[sid] = {name = s.name, unit = s.unit, prec = s.prec, min = s.min, max = s.max, dec = decFn}
            end
        end
    end
...
local function isEnabled(sidElrs) return enabledSidElrs[sidElrs] == true end

local sensors = {uid = {}, lastvalue = {}, lasttime = {}}

local constants = {CRSF_FRAME_CUSTOM_TELEM = 0x88, FRAME_COUNT_ID = 0xEE01, FRAME_SKIP_ID = 0xEE02}
...
local function resetSensors() sensors.uid, sensors.lastvalue, sensors.lasttime = {}, {}, {} end
...
local function getOrCreateSensor(uid, name, unit, dec, value, min, max)
    if not sensors.uid[uid] then
        ...
        if not sensors.uid[uid] then createTelemetrySensor(uid, name, unit, dec, value, min, max) end
    end
...
function elrs.crossfirePop()
    ...
    if command == constants.CRSF_FRAME_CUSTOM_TELEM then
        ...
        while ptr < len do
            ...
            if val ~= nil then setTelemetryValue(sidElrs, 0, 0, val, sensor.unit, sensor.prec, sensor.name, sensor.min, sensor.max) end
...
function elrs.reset() resetSensors() end
