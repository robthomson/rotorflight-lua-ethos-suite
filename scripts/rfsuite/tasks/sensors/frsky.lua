...
local function telemetryActive() return rfsuite and rfsuite.session and rfsuite.session.telemetryState == true end
...
local function hasPendingActions() return next(createSensorList) or next(renameSensorList) end
    ...
    for _, id in ipairs(fblIds or {}) do
        ...
        if s and s.sidSport then
            ...
            if type(sport) == "table" then
                enabledAppIds[sport[1]] = true
            else
                enabledAppIds[sport] = true
            end
        end
    end
        ...
        if sport then
            ...
            if enabledAppIds[triggerAppId] then
                ...
                createSensorList[triggerAppId] = {
                    ...
                    extras = (function()
                        local acc = {}
                        for i = 2, #sport do acc[#acc + 1] = {appId = sport[i], name = names[i] or (s.name .. " #" .. i), unit = s.unit, decimals = (s.sportDecimals ~= nil) and s.sportDecimals or s.prec, minimum = s.min, maximum = s.max} end
                        return (#acc > 0) and acc or nil
                    end)()
                }

                if s.sportRename then renameSensorList[triggerAppId] = (s.sportRename[1] and s.sportRename) or {s.sportRename} end
            end
        end
    end
            ...
            if v.extras then
                for _, e in ipairs(v.extras) do
                    ...
                    if not existing then
                        ...
                        if e.minimum ~= nil then
                            sExtra:minimum(e.minimum)
                        else
                            sExtra:minimum(-1000000000)
                        end
                        if e.maximum ~= nil then
                            sExtra:maximum(e.maximum)
                        else
                            sExtra:maximum(2147483647)
                        end
                        if e.unit ~= nil then
                            sExtra:unit(e.unit);
                            sExtra:protocolUnit(e.unit)
                        end
                        if e.decimals ~= nil then
                            sExtra:decimals(e.decimals);
                            sExtra:protocolDecimals(e.decimals)
                        end
                    end
                end
            end

            createSensorList[appId] = nil
            return "created"
        end
    end
    return "noop"
...
local function telemetryPop()
    ...
    if not (createSensorList[appId] or renameSensorList[appId]) then return true end

    local cs = createSensor(physId, primId, appId, value)
    if cs == "created" then return true end

    if renameSensorList[appId] then renameSensor(physId, primId, appId, value) end
