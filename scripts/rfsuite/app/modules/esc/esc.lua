...
local function openPage(pidx, title, script)
    ...
    for pidx, pvalue in ipairs(pages) do
        ...
        if rfsuite.config.iconSize ~= 0 then
            if rfsuite.app.gfx_buttons["escmain"][pidx] == nil then rfsuite.app.gfx_buttons["escmain"][pidx] = lcd.loadMask("app/modules/esc/mfg/" .. pvalue.folder .. "/" .. pvalue.image) end
        else
        ...
        rfsuite.app.formFields[pidx] = form.addButton(line, {x = bx, y = y, w = buttonW, h = buttonH}, {
            ...
            press = function()
                ...
                rfsuite.app.ui.openPage(pidx, pvalue.folder, "esc/esc_tool.lua")
            end
        })
