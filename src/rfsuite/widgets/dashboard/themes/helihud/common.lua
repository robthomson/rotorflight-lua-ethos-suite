...
local CURRENT_ALIAS_KEYS = {"current", "esc_current", "current_meter", "curr", "esc1_current", "amperage", "amps"}
local CURRENT_ALIAS_SOURCE_CANDIDATES = {
...
local currentAliasSources = {}
local TRACKED_ESC_TEMP_SPEC = {source = "temp_esc"}
...
local function readArmflags(telemetry, showReason)
    ...
    if (floor(value) & 1) == 1 then return "ARMED" end
    return "DISARMED"
end
...
local function readTrackedEscTempMax(spec, telemetry)
    ...
    TRACKED_ESC_TEMP_SPEC.thresholds = spec.thresholds
    local liveValue, _, dynamicUnit, _, _, thresholds = readTelemetry(TRACKED_ESC_TEMP_SPEC, telemetry)
...
local function readCurrentAlias(telemetry)
    ...
    if telemetry and telemetry.getSensor then
        for i = 1, #CURRENT_ALIAS_KEYS do
            local v = telemetry.getSensor(CURRENT_ALIAS_KEYS[i])
            if type(v) == "number" then return v end
        end
    end

    if system and system.getSource then
        for i = 1, #CURRENT_ALIAS_SOURCE_CANDIDATES do
            local src = currentAliasSources[i]
            if src == nil then
                local ok, resolved = pcall(system.getSource, CURRENT_ALIAS_SOURCE_CANDIDATES[i])
                src = ok and resolved or false
                currentAliasSources[i] = src
            end
