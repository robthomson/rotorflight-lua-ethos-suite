...
local function paintAesGauge(x, y, w, h, box, cache)
    ...
    lcd.color(rc.dim)
    for i = 0, 10 do
        local a = startAngle + (sweep * (i / 10))
        if a >= 360 then a = a - 360 end
        local radA = math.rad(a - 90)
        local tx1 = cx + math.cos(radA) * (rOuter + 3)
        local ty1 = cy + math.sin(radA) * (rOuter + 3)
        local tx2 = cx + math.cos(radA) * (rOuter + 9)
        local ty2 = cy + math.sin(radA) * (rOuter + 9)
        lcd.drawLine(tx1, ty1, tx2, ty2)
    end
