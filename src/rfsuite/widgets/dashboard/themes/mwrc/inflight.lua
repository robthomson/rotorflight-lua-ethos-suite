...
local function drawVerticalBatteryBar(x, y, w, h, percent, fillbgcolor, fillcolor, accentcolor, frameThickness, cappaddingtop)
    ...
    local isCrit = ((percent or 0) <= 0.25)
...
local function rightStackPaint(x, y, w, h, box, c)
    ...
    -- Use dynamic gradient for battery bar
    local safeFuelColor = getBatteryColor(c.fuelPercent or 0)
...
local function buildBoxes(W)
    ...
    return {
        ...
        -- THE FLAWLESS NO-OVERLAP GRID: Gauges locked to individual columns. No offsets used.
        {
            -- SPREAD GAUGE 1: ESC TEMP (Left Column)
        ...
        {
            -- SPREAD GAUGE 2: THROTTLE (Center Column)
        ...
        {
            -- SPREAD GAUGE 3: HEADSPEED (Right Column)
