--[[
  Copyright (C) 2025 Rotorflight Project
  GPLv3 — https://www.gnu.org/licenses/gpl-3.0.en.html
]] --

local rfsuite = require("rfsuite")
local system = system
local flightState = (rfsuite.shared and rfsuite.shared.flight) or assert(loadfile("shared/flight.lua"))()
local servoState = (rfsuite.shared and rfsuite.shared.servo) or assert(loadfile("shared/servo.lua"))()


-- Optimized locals to reduce global/table lookups
local utils = rfsuite.utils
local helpers = {}

function helpers.governorMode(callback)
    local currentMode = flightState.getGovernorMode()

    if currentMode == nil then
        local msp = rfsuite.tasks.msp
        local API = msp and msp.api.load("GOVERNOR_CONFIG")
        if API and API.enableDeltaCache then API.enableDeltaCache(false) end
        API.setCompleteHandler(function(self, buf)
            local governorMode = API.readValue("gov_mode")
            if governorMode then 
                utils.log("Governor mode: " .. governorMode, "debug") 
            end
            if callback then callback(flightState.setGovernorMode(governorMode)) end
        end)
        API.setUUID("e2a1c5b3-7f4a-4c8e-9d2a-3b6f8e2d9a1c")
        API.read()
    else
        if callback then callback(currentMode) end
    end
end

function helpers.servoCount(callback)
    local currentCount = servoState.getCount()
    if currentCount == nil then
        local msp = rfsuite.tasks.msp
        local API = msp and msp.api.load("STATUS")
        if API and API.enableDeltaCache then API.enableDeltaCache(false) end
        API.setCompleteHandler(function(self, buf)
            local servoCount = servoState.setCount(API.readValue("servo_count"))
            if servoCount then 
                utils.log("Servo count: " .. servoCount, "debug") 
            end    
            if callback then callback(servoCount) end
        end)
        API.setUUID("d7e0db36-ca3c-4e19-9a64-40e76c78329c")
        API.read()
    else
        if callback then callback(currentCount) end    
    end
end

function helpers.servoOverride(callback)
    local currentOverride = servoState.getOverride()
    if currentOverride == nil then
        local msp = rfsuite.tasks.msp
        local API = msp and msp.api.load("SERVO_OVERRIDE")
        if API and API.enableDeltaCache then API.enableDeltaCache(false) end
        API.setCompleteHandler(function(self, buf)
            local override = nil
            for i, v in pairs(API.data().parsed) do
                if v == 0 then
                    utils.log("Servo override: true (" .. i .. ")", "debug")
                    override = true
                end
            end
            if override == nil then override = false end
            override = servoState.setOverride(override)
            if callback then callback(override) end
        end)
        API.setUUID("b9617ec3-5e01-468e-a7d5-ec7460d277ef")
        API.read()
    else
        if callback then callback(currentOverride) end    
    end
end


function helpers.servoBusEnabled(callback)

    local FBUS_FUNCTIONMASK = 524288
    local SBUS_FUNCTIONMASK = 262144

    local function processSerialConfig(data) 
        for i, v in ipairs(data) do 
            if v.functionMask == FBUS_FUNCTIONMASK then 
                return  true 
            end 
            if v.functionMask == SBUS_FUNCTIONMASK then 
                return  true 
            end             
        end 
        return false
    end

    local currentBusEnabled = servoState.getBusEnabled()
    if currentBusEnabled == nil then
        local message = {
            command = 54,
            processReply = function(self, buf)
                local data = {}

                buf.offset = 1
                for i = 1, 6 do
                    data[i] = {}
                    data[i].identifier = rfsuite.tasks.msp.mspHelper.readU8(buf)
                    data[i].functionMask = rfsuite.tasks.msp.mspHelper.readU32(buf)
                    data[i].msp_baudrateIndex = rfsuite.tasks.msp.mspHelper.readU8(buf)
                    data[i].gps_baudrateIndex = rfsuite.tasks.msp.mspHelper.readU8(buf)
                    data[i].telemetry_baudrateIndex = rfsuite.tasks.msp.mspHelper.readU8(buf)
                    data[i].blackbox_baudrateIndex = rfsuite.tasks.msp.mspHelper.readU8(buf)
                end

                local servoBusEnabled = servoState.setBusEnabled(processSerialConfig(data))
                if callback then callback(servoBusEnabled) end
            end,
            simulatorResponse = {20 , 1  , 0  , 0  , 0  , 5  , 4  , 0  , 5  , 0  , 0  , 0  , 8  , 0  , 5  , 4  , 0  , 5  , 1  , 0  , 4  , 0  , 0  , 5  , 4  , 0  , 5  , 2  , 0  , 0  , 0  , 0  , 5  , 4  , 0  , 5  , 3  , 0  , 0  , 0  , 0  , 5  , 4  , 0  , 5  , 4  , 64 , 0  , 0  , 0  , 5  , 4  , 0  , 5  , 5  , 0  , 0  , 0  , 0  , 5  , 4  , 0  , 5  }
        }
        rfsuite.tasks.msp.mspQueue:add(message)
    else
        if callback then callback(currentBusEnabled) end
    end
end

function helpers.mixerConfig(callback)
    local tailMode = flightState.getTailMode()
    local swashMode = flightState.getSwashMode()

    if tailMode == nil or swashMode == nil then
        local msp = rfsuite.tasks.msp
        local API = msp and msp.api.load("MIXER_CONFIG")
        if API and API.enableDeltaCache then API.enableDeltaCache(false) end
        API.setCompleteHandler(function(self, buf)
            local resolvedTailMode = API.readValue("tail_rotor_mode")
            local resolvedSwashMode = API.readValue("swash_type")
            if system and system.getVersion and system.getVersion().simulation then
                local dev = rfsuite.preferences and rfsuite.preferences.developer
                local override = dev and dev.tailmode_override
                override = tonumber(override)
                if override == 0 or override == 1 then
                    resolvedTailMode = override
                    utils.log("Tail mode override (developer): " .. tostring(override), "debug")
                end
            end
            resolvedTailMode, resolvedSwashMode = flightState.setMixerConfig(resolvedTailMode, resolvedSwashMode)
            if resolvedTailMode and resolvedSwashMode then
                utils.log("Tail mode: " .. resolvedTailMode, "debug")
                utils.log("Swash mode: " .. resolvedSwashMode, "debug")
            end
            if callback then callback(resolvedTailMode, resolvedSwashMode) end
        end)
        API.setUUID("fbccd634-c9b7-4b48-8c02-08ef560dc515")
        API.read()
    else
        if callback then callback(tailMode, swashMode) end
    end
end

function helpers.tailMode(callback)
    helpers.mixerConfig(function(tailMode, swashMode)
        if callback then callback(tailMode) end
    end)
end

function helpers.swashMode(callback)
    helpers.mixerConfig(function(tailMode, swashMode)
        if callback then callback(swashMode) end
    end)
end


return helpers
