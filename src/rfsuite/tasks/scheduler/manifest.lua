...
local manifest = {
  ...
  [8]  = { name = "telemetry",    connected = false, interval = 0.52, script = "scheduler/telemetry/telemetry.lua",        linkrequired = false, simulatoronly = false, spreadschedule = true  },

  -- Developer utilities (disabled by default)
  [9]  = { name = "developer",    connected = false, interval = -1,   script = "scheduler/developer/developer.lua",        linkrequired = false, simulatoronly = true,  spreadschedule = true  },

  -- Log output task; internally rate-limited
  [10] = { name = "logger",       connected = false, interval = 0.28, script = "scheduler/logger/logger.lua",              linkrequired = false, simulatoronly = false, spreadschedule = true  },

  -- Sensor polling; runs frequently to keep data fresh
  [11] = { name = "sensors",      connected = true,  interval = 0.18, script = "scheduler/sensors/sensors.lua",            linkrequired = true,  simulatoronly = false, spreadschedule = false },

  -- Timer service; must tick faster than 0.5s for accuracy
  [12] = { name = "timer",        connected = true,  interval = 0.25, script = "scheduler/timer/timer.lua",                linkrequired = true,  simulatoronly = false, spreadschedule = false },

  -- MSP processing; adaptive internally, with boost during active traffic
  [13] = { name = "msp",          connected = false, interval = 0.25,  script = "scheduler/msp/msp.lua",                    linkrequired = false, simulatoronly = false, spreadschedule = false },
}
