...
local supportedProtocols = {
  sport = {
    ...
    saveTimeout        = 10.0,        -- Timeout (seconds) for MSP save/write operations
    pageReqTimeout     = 15,          -- Timeout when requesting UI/CMS pages from the FC
  ...
  crsf = {
    ...
    saveTimeout        = 10.0,        -- Timeout (seconds) for MSP save/write operations
    pageReqTimeout     = 10,          -- Timeout when requesting CMS pages via CRSF
...
function protocol.getTransports()
    ...
    for i, v in pairs(supportedProtocols) do
        transport[i] = "SCRIPTS:/" .. rfsuite.config.baseDir .. "/tasks/msp/" .. v.mspTransport
    end
    return transport
end
