...
local CORE_SINGLETON_KEY = "rfsuite.shared.mspapi.core"

if package.loaded[CORE_SINGLETON_KEY] then
    return package.loaded[CORE_SINGLETON_KEY]
end
...
local function getMspHelper()
    ...
    return msp and msp.mspHelper or nil
end
    ...
    if chunked then
        ...
        local function processNextChunk()
            ...
            while state.index <= #structure and processedFields < fieldsPerTick do
                ...
                readFunction = getMspHelper() and getMspHelper()["read" .. field.type] or nil
                ...
                startByte = state.currentByte
                endByte = startByte + size - 1
...
function core.buildDeltaPayload(apiname, payload, api_structure, positionmap, receivedBytes, receivedBytesCount)
    ...
    for _, field_def in ipairs(api_structure) do
        ...
        value = payload[name] or field_def.default or 0
        scale = field_def.scale or 1
        value = math_floor(value * scale + 0.5)

        mspHelper = getMspHelper()
        writeFunction = mspHelper["write" .. field_def.type]
...
package.loaded[CORE_SINGLETON_KEY] = core
